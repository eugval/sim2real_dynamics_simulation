...
class ReplayBufferPUP:
    """
    ReplayBuffer for Projected Universal Policy
    """
    def __init__(self, capacity):
        self.capacity = capacity
        self.buffer = []
    ...
    def push(self, state, action, reward, next_state, done, params):
        if len(self.buffer) < self.capacity:
            self.buffer.append(None)
        self.buffer[self.position] = (state, action, reward, next_state, done, params)
        self.position = int((self.position + 1) % self.capacity)  # as a ring buffer

    def sample(self, batch_size):
        batch = random.sample(self.buffer, batch_size)
        state, action, reward, next_state, done, params = map(np.stack,
                                                      zip(*batch))  # stack for each element
        ''' 
        the * serves as unpack: sum(a,b) <=> batch=(a,b), sum(*batch) ;
        zip: a=[1,2], b=[2,3], zip(a,b) => [(1, 2), (2, 3)] ;
        ...
        np.stack((1,2)) => array([1, 2])
        '''
        return state, action, reward, next_state, done, params

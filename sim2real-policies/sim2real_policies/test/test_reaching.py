import numpy as np
from robo_extra.env_base import make
...
def testing_envionments_generator_random(env, number_of_envs):
    ...
    for i in range(0, number_of_envs):
        ...
        if (uses_pid):
            kps = default_parameters['kps'] * parameter_ranges['gains'][0] + _FIXED_RANDOM_NUMBERS[i, 2] * \
                  parameter_range_len['gains']
            kis = default_parameters['kis'] * parameter_ranges['gains'][0] + _FIXED_RANDOM_NUMBERS[i, 3] * \
                  parameter_range_len['gains']
            kds = default_parameters['kds'] * parameter_ranges['gains'][0] + _FIXED_RANDOM_NUMBERS[i, 4] * \
                  parameter_range_len['gains']
            ...
            for j in range(7):
                ...
                params[gain_name] = default_parameters[gain_name] * \
                                    parameter_ranges['gains'][0] + _FIXED_RANDOM_NUMBERS[i, 2+ j] * \
                                    parameter_range_len['gains']
